...
onnx_model = onnx.load('./kp_detector.onnx')
...
def _convert_resize(builder, node, graph, err):
    ...
    if len(scale) == 0:
        ...
        if up_counter == 0:
            builder.add_resize_bilinear(node.name, node.inputs[0], node.outputs[0], target_height=4, target_width=4)
            up_counter+=1
            return
        if up_counter == 1:
            builder.add_resize_bilinear(node.name, node.inputs[0], node.outputs[0], target_height=8, target_width=8)
            up_counter+=1
            return
        if up_counter == 2:
            builder.add_resize_bilinear(node.name, node.inputs[0], node.outputs[0], target_height=16, target_width=16)
            up_counter+=1
            return
        if up_counter == 3:
            builder.add_resize_bilinear(node.name, node.inputs[0], node.outputs[0], target_height=32, target_width=32)
            up_counter+=1
            return
        if up_counter == 4:
            builder.add_resize_bilinear(node.name, node.inputs[0], node.outputs[0], target_height=64, target_width=64)
...
mlmodel = convert(model='./kp_detector.onnx',custom_conversion_functions=custom_conversion_functions,add_custom_layers=True,minimum_ios_deployment_target='13')
